		...
		<view class="oth">
			<swiper :vertical="true" @change='swiperChange'>
				<swiper-item class="mitem" v-for="(item,index) in mvList" :key="index" v-if="mvList!==[]">
					<video custom-cache="{undefined{false}}" :id="'video'+index" :src="item.mvUrl"
						 controls :show-fullscreen-btn="false" :page-gesture="true"
						:class="{changeTop:(isShow && index===swiperIndex),hide:(index!==swiperIndex)}" :enable-play-gesture="true"></video>
					<!-- mv信息 -->
					<view class="intro">
						<view class="singer">
					...
					<scroll-view scroll-y="true" v-if="(isShow && index===swiperIndex)" :lower-threshold="150"
